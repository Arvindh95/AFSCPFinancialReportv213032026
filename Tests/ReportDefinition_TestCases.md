...
**Date:** 2026-04-17
**Version:** AFSCPFinancialReport v2.1
...
### A-09: Detect Columns — Button Disabled When GI Blank
**Objective:** Verify Detect Columns button only enabled when GI Name is filled.

**Steps:**
1. Open a definition with GI Name blank
2. Observe **Detect Columns** button state

**Expected Result:**
- Button is **disabled** when GI Name is blank
- Button becomes **enabled** when GI Name is filled
...
### A-10: Detect Columns — Successful Detection
**Objective:** Verify Detect Columns fetches column names from GI.

**Steps:**
1. Set GI Name = `TrialBalance`
2. Click **Detect Columns**

**Expected Result:**
- Dialog shows detected column names (Account, Type, BeginningBalance, EndingBalance, Debit, Credit, etc.)
- Column mapping dropdowns now populated with detected column names
...
**Expected Result:**
- Value is **positive** — engine normalizes L/I/Q from credit (negative GL) to positive via `ApplyAccountTypeSign`
...
**Expected Result:**
- All account types (A, L, E, I, Q) in range included
...
**Steps:**
1. Balance Type = `Period Debit`, run for Jun 2025
2. Compare to Debit (YTD) for same period

**Expected Result:**
- Period Debit = only Jun 2025 debit
- Must be <= YTD Debit (YTD includes Jan–Jun)
...
**Steps:**
1. Balance Type = `Period Credit`, run for Jun 2025

**Expected Result:**
- Only Jun 2025 credit column value
...
**Steps:**
1. Balance Type = `Period Movement`, run for Jun 2025

**Expected Result:**
- Jun 2025 Debit − Jun 2025 Credit (single month only)
...
**Steps:**
1. Create:
   - `NET_PROFIT` (Calculated), Formula = `REVENUE - OPEX`, ParentLineCode = `TOTAL_EQUITY`
   - `RETAINED_EARNINGS` (Account), ParentLineCode = `TOTAL_EQUITY`
   - `TOTAL_EQUITY` (Subtotal)
2. Run report

**Expected Result:**
- `TOTAL_EQUITY = NET_PROFIT + RETAINED_EARNINGS`
...
**Expected Result:**
- Error thrown: *"Circular dependency detected: BS_LINE_A, BS_LINE_B"*
...
### G-05: Copy Definition
**Objective:** Verify Copy Definition creates complete copy.

**Prerequisites:** Definition `TEST-BS` exists with multiple lines.

**Steps:**
1. Open `TEST-BS`
2. Click **Copy Definition** → confirm dialog
3. Navigate to new definition

**Expected Result:**
- New DefinitionCD = `TEST-BS_COPY`
- New Prefix = original prefix truncated to 7 chars + `CP`
- All line items copied with same LineCode, ParentLineCode, Formula, all fields intact
- Both DefinitionCD and Prefix locked on new record
...
**Expected Result:**
- Error: *"Duplicate prefix in report: BS"*
...
**Expected Result:**
- Either blocked at save (G-02, G-08) or caught at calculation time with error: *"Duplicate global key detected: BS_CASH"*
...
| 130 | RETAINED | Account | 30100 | 30199 | TOTAL_EQUITY | — |
| 140 | NET_PROFIT | Calculated | — | — | TOTAL_EQUITY | `REVENUE - OPEX` |
| 150 | TOTAL_EQUITY | Subtotal | — | — | — | — |
| 160 | CHECK | Calculated | — | — | — | `TOTAL_ASSETS - TOTAL_LIAB - TOTAL_EQUITY` |

3. Run report for a valid period

**Expected Result:**
- All lines calculate correctly
- `CHECK` = 0 (balance sheet balances)
- `HDG_ASSETS` and `HDG_LIAB` = empty
- `NET_PROFIT` (Calculated) included in `TOTAL_EQUITY` subtotal
