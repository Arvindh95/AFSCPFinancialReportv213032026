...
| `Template contains {0} placeholders. Maximum allowed is {1}.`                                | Defined (`Messages.TooManyPlaceholders` + `Constants.MaxPlaceholdersPerTemplate` = 1000) but **not currently enforced** in the generation path — this error is never actually raised. | No action; there is no live placeholder-count cap. |
